(() => {
  ...
  const STORAGE_KEY = 'acompanhamento-escolar-demo-v2';
  const TERMS = ['1', '2', '3'];
  ...
  function renderProfessionalAttendance() { $('#profAttendanceTable').innerHTML = [{date:'08/08/2026',teacher:'Professor 01',subject:'Planejamento de recuperação',status:'Em acompanhamento'},{date:'02/08/2026',teacher:'Professor 02',subject:'Acompanhamento de turma',status:'Concluído'},{date:'29/07/2026',teacher:'Professor 03',subject:'Estratégias de participação',status:'Registrado'}].map(x => `<tr><td>${x.date}</td><td><strong>${x.teacher}</strong></td><td>${x.subject}</td><td><span class="status-pill ${x.status === 'Concluído' ? 'status-active' : 'status-pending'}">${x.status}</span></td></tr>`).join(''); }
